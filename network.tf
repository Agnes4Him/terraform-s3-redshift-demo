...
resource "aws_vpc" "redshift-serverless-vpc" {
  ...
  tags = {
    Name = "${var.env_prefix}-redshift-serverless-vpc"
  }
}
...
resource "aws_subnet" "redshift-serverless-subnet-az1" {
  ...
  tags = {
    Name = "${var.env_prefix}-redshift-serverless-subnet-az1"
  }
}
...
resource "aws_subnet" "redshift-serverless-subnet-az2" {
  ...
  tags = {
    Name = "${var.env_prefix}-redshift-serverless-subnet-az2"
  }
}
...
resource "aws_subnet" "redshift-serverless-subnet-az3" {
  ...
  tags = {
    Name = "${var.env_prefix}-redshift-serverless-subnet-az3"
  }
}